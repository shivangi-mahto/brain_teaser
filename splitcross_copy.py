...
class SplitCrossEntropyLoss(nn.Module):
    ...
    def forward(self, weight, bias, hiddens, targets, verbose=False):
        ...
        #print('The length of predicted probs',len(softmaxed_all_head_res))
        #print((softmaxed_all_head_res))
        #exit(0)
        if self.verbose or verbose:
        ...
        for idx in range(self.nsplits):
            ...
            #print(entropy)
            #print('Entropy sixe', entropy.shape, len(targets))
            total_loss = entropy.float() if total_loss is None else total_loss + entropy.float()                                
            #total_loss = entropy.float().sum() if total_loss is None else total_loss + entropy.float().sum()
            #print(total_loss.shape)
            length = len(targets)
        return (total_loss).type_as(weight) #divide by len target for high low perfor but not for model abl perof
